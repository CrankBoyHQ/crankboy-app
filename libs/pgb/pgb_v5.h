...
/* DEVELOPMENT VERSION RANGE: (?, v2.2.1] */
...
char* savestate_upgrade_to_v5(char** out, size_t* out_size, char* in, size_t in_size)
{
    ...
    for (int ch = 0; ch < 4; ch++)
    {
        ...
        v5_gb->audio.chans[ch].sweep.did_subtract = false;
    ...
    v5_header->version = PGB_VERSION;
    v5_header->gb_s_size = v5_gb_size;

    if (extra_sz)
        memcpy(
            v5_org + sizeof(StateHeader) + v5_gb_size,
            v4_in + sizeof(StateHeader) + sizeof(struct gb_s_v4), extra_sz
        );
