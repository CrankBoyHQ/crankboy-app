...
static void CB_ParentalLockScene_update(CB_ParentalLockScene* parentalLockScene, uint32_t u32enc_dt)
{
    ...
    if (parentalLockScene->sel == 4)
    {
        if (CB_App->buttons_pressed & kButtonA)
        {
            ...
            {
                if (memcmp(parentalLockScene->lock_compare, parentalLockScene->lock_value, 4*sizeof(unsigned)) != 0)
                {
                ...
                else
                {
                    ...
                    CB_Modal* modal = CB_Modal_new(
                        "Unlock temporarily or permanently?",
                        options, (void*)parental_lock_disengage, parentalLockScene
                    );
                    modal->width = 330;
                    modal->height += 20;
            ...
            else
            {
                ...
                CB_Modal* modal = CB_Modal_new(
                    "Really set parental lock? Internet features will be restricted until the lock is disengaged. If you forget the password, you can simply delete " PARENTAL_LOCK_FILE " as a fallback.",
    ...
    if (parentalLockScene->unlocking)
    {
        playdate->graphics->drawText("Please enter the code, or Ⓑ to cancel.", 50, kUTF8Encoding, 50, 50);
    }
    else
    {
        playdate->graphics->drawText("Please enter a code, or Ⓑ to cancel.", 50, kUTF8Encoding, 50, 50);
    }
}
