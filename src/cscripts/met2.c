...
static ScriptData* on_begin(gb_s* gb, char* header_name)
{
    ...
    LCDBitmap* htimg =
        playdate->graphics->loadBitmap(CB_get_forwarded_path(MET2_ASSETS_DIR "pdimg"), NULL);
    if (!htimg)
        return NULL;
    ...
    data->htimg = htimg;
    data->heximg =
        playdate->graphics->loadBitmap(CB_get_forwarded_path(MET2_ASSETS_DIR "hex"), NULL);
    data->heximg_dark =
        playdate->graphics->loadBitmap(CB_get_forwarded_path(MET2_ASSETS_DIR "hexdark"), NULL);
    ...
    data->glyphs = split_subimages(
        playdate->graphics->loadBitmap(CB_get_forwarded_path(MET2_ASSETS_DIR "font"), NULL), 16, 16,
        &data->glyph_c
    );
